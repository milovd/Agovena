...
test('mobile navigation keeps search and cart in the header with a collapsed account drawer', function () {
    ...
    expect($drawerStart)->toBeInt()
        ...
        ->and(substr_count($html, 'x-data="storefrontHeader"'))->toBe(1)
        ->and(substr_count($html, 'x-data="storefrontDisclosure"'))->toBeGreaterThanOrEqual(2)
        ->and(substr_count($html, '@click="toggleNav()"'))->toBe(1)
        ->and(substr_count($html, '@keydown.escape.window="closeAll()"'))->toBe(1)
        ->and(substr_count($html, '@click="toggleMobileCategory('))->toBeGreaterThan(0)
        ...
        ->and(substr_count($html, 'x-effect="syncDrawerLock()"'))->toBe(1);
});
