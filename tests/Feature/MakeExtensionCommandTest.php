...
test('make-extension scaffolds a valid category instead of integration', function () {
    ...
    $root = optionalExtensionRoot('payments', $id);
...
test('make-extension rejects unknown categories', function () {
    ...
    expect(File::isDirectory(optionalExtensionRoot('bad-category-example')))->toBeFalse()
        ->and(File::isDirectory(optionalExtensionRoot('other', 'bad-category-example')))->toBeFalse();
});
