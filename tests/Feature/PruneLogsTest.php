...
test('prune logs removes expired operational rows and leaves financial orders', function () {
    ...
    PaymentWebhookEvent::query()->create([
        'gateway_id' => 'tebex',
        'external_event_id' => 'evt-tebex-ledger',
        'status' => 'paid',
        'processing_status' => 'processed',
        'retention_exempt' => true,
    ]);
    PaymentWebhookEvent::query()->where('external_event_id', 'evt-tebex-ledger')->update([
        'created_at' => now()->subDays(120),
        'updated_at' => now()->subDays(120),
    ]);
    ...
    expect(DB::table('email_logs')->where('to', 'old@example.test')->exists())->toBeFalse()
        ...
        ->and(PaymentWebhookEvent::query()->where('external_event_id', 'evt-tebex-ledger')->exists())->toBeTrue();
});
