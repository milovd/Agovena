<?php

declare(strict_types=1);

return [
    'skip_to_content' => 'Skip to content',
    'shop' => 'Shop',
    'meta_description' => 'Self-hosted commerce for products, services, domains, and subscriptions.',
    'cookie_consent' => [
        'eyebrow' => 'Privacy',
        'title' => 'We use cookies',
        'lead' => 'We use cookies to keep this site usable and, if you allow it, to analyse our traffic. Information about your use of this site may be shared with the configured analytics software, which may combine it with other information you have provided to that service or that it has collected from your use of its services. This site does not use marketing or social-media advertising cookies.',
        'policy' => 'Cookie policy',
        'privacy' => 'Privacy',
        'reject' => 'Essential only',
        'customize' => 'Customize choice',
        'accept' => 'Accept all',
        'close' => 'Close',
        'tab_consent' => 'Consent',
        'tab_about' => 'About',
        'essential' => 'Necessary',
        'essential_description' => 'Necessary cookies help make a website usable by enabling basic functions like page navigation and access to secure areas of the website. The website cannot function properly without these cookies.',
        'essential_locked' => 'Necessary cookies cannot be switched off. They store your language preference and cookie choice so the site can work and remember your decision. No consent is required for these cookies under GDPR.',
        'analytics' => 'Analytics',
        'analytics_description' => 'Analytics cookies help Agovena understand how visitors use this site (pages viewed, approximate location, device/browser, and referral source). They are provided by the configured analytics software and load only after you opt in.',
        'save' => 'Save preferences',
        'settings_link' => 'Cookie settings',
        'about_paragraphs' => [
            'Cookies are small text files that websites can use to make a user’s experience more efficient.',
            'The law allows storing cookies on your device if they are strictly necessary for this site to operate. For all other cookies, Agovena needs your permission. Necessary cookies on this site are processed on the basis of GDPR Art. 6 (1) (f). Analytics cookies are processed on the basis of GDPR Art. 6 (1) (a) (consent).',
            'Agovena uses necessary cookies and may use a configured bot-protection solution on the contact form. Only if you consent, it uses analytics cookies from the configured analytics software. There are no marketing cookie categories for Agovena.',
            'You can change or withdraw your analytics consent at any time via Customize choice in the footer or on the cookie policy page.',
            'Learn more about Agovena, how to contact us, and how personal data is processed in the privacy policy.',
            'When you contact Agovena about your consent, please include your consent ID and date shown below.',
        ],
        'consent_meta_empty' => 'No consent choice saved yet.',
        'consent_meta' => 'Consent ID: :id · Date: :date',
        'saved_choice' => 'Your cookie preference was saved.',
        'save_error' => 'Your preference could not be saved. Please try again.',
    ],
    'benefits_aria' => 'Store benefits',
    'primary_nav' => 'Primary',
    'mobile_nav' => 'Mobile',
    'menu' => 'Menu',
    'close' => 'Close',
    'breadcrumb_aria' => 'Breadcrumb',

    'nav' => [
        'home' => 'Home',
        'categories' => 'Categories',
        'cart' => 'Cart',
        'account' => 'Account',
        'account_menu' => 'Account menu',
        'login' => 'Log in',
        'register' => 'Register',
        'dashboard' => 'Dashboard',
        'orders' => 'Orders',
        'admin' => 'Admin',
        'admin_hint' => 'Open the Admin for this store',
        'logout' => 'Log out',
        'deals' => 'Deals',
        'about' => 'About',
        'terms' => 'Terms',
        'privacy' => 'Privacy',
        'shop_products' => 'Products',
        'browse_categories' => 'Browse categories',
        'all_categories' => 'All categories',
        'shop_all' => 'Shop all :name',
        'browse_collection' => 'Browse this collection for featured products.',
    ],

    'preferences' => [
        'aria' => 'Store preferences',
        'region' => 'Language and currency',
        'language' => 'Language',
        'currency' => 'Currency',
        'theme_light' => 'Light',
        'theme_dark' => 'Dark',
        'theme_to_light' => 'Switch to light mode',
        'theme_to_dark' => 'Switch to dark mode',
        'currency_cart_cleared' => 'Your cart was cleared because it used a different currency. Showing :currency.',
    ],

    'search' => [
        'label' => 'Search products',
        'placeholder' => 'Search products',
        'clear' => 'Clear search',
        'suggestions' => 'Search suggestions',
        'searching' => 'Searching…',
        'no_matches' => 'No matches',
        'view_all' => 'View all results',
        'heading' => 'Search',
        'results_for' => 'Results for “:query”',
        'no_products' => 'No products match your search',
        'try_different' => 'Try a different term, or <a href=":url">clear search</a>.',
    ],

    'cart' => [
        'title' => 'Cart',
        'empty_title' => 'Your cart is empty',
        'empty_text' => 'Browse the catalog and add items when you are ready.',
        'continue_shopping' => 'Continue shopping',
        'each' => 'each',
        'quantity_for' => 'Quantity for :name',
        'summary' => 'Summary',
        'summary_aria' => 'Order summary',
        'subtotal' => 'Subtotal',
        'checkout' => 'Checkout',
        'remove_item' => 'Remove :name',
        'items' => '{1} :count item|[2,*] :count items',
    ],

    'checkout' => [
        'title' => 'Checkout',
        'kicker' => 'Complete your order',
        'lede' => 'Guest checkout - no customer account required.',
        'lede_account' => 'You are signed in. This order will appear in your account.',
        'progress_aria' => 'Checkout progress',
        'step_of' => 'Step :current of :total · :label',
        'step_completed' => 'Completed',
        'step_current' => 'Current step',
        'step_upcoming' => 'Upcoming',
        'steps' => [
            'details' => 'Information',
            'delivery' => 'Delivery',
            'configuration' => 'Configure',
            'fulfillment' => 'Delivery & configure',
            'payment' => 'Payment',
            'review' => 'Completed',
        ],
        'back' => 'Back',
        'back_to_cart' => 'Back to cart',
        'back_to_cart_short' => 'Cart',
        'summary_qty_price' => ':qty × :price',
        'continue' => 'Continue',
        'continue_to' => 'Continue to :step',
        'continue_to_secure_payment' => 'Continue to payment',
        'continue_to_provider' => 'Continue to :provider',
        'pay_amount' => 'Pay :amount',
        'working' => 'Working…',
        'edit' => 'Edit',
        'fix_errors' => 'Please correct the highlighted fields.',
        'updating_shipping' => 'Updating delivery options…',
        'shipping_fallback' => 'If a carrier is unavailable, configured store rates still apply when they exist.',
        'hosted_payment_note' => 'Card details are entered with the payment provider. Agovena does not collect card numbers.',
        'renewal_mode' => 'How should this service renew?',
        'renewal_manual' => 'Manual renewal',
        'renewal_manual_help' => 'We create an invoice before the renewal date. You choose the payment method and pay it yourself.',
        'renewal_automatic' => 'Automatic renewal',
        'renewal_automatic_help' => 'Mollie uses the authorised payment mandate for future renewals when available.',
        'pay_with_account_balance' => 'Pay with account balance',
        'account_balance_available' => 'Available: :amount',
        'no_payment_methods' => 'No payment methods are available right now.',
        'configuration_note' => 'These choices were set when the items were added. Return to the cart to change them.',
        'contact' => 'Contact information',
        'additional_details' => 'Additional details',
        'name' => 'Name',
        'email' => 'Email',
        'email_hint' => 'We’ll send the order confirmation here.',
        'billing' => 'Billing address',
        'shipping' => 'Shipping address',
        'saved_address' => 'Use a saved address',
        'address_suggestions' => 'Address suggestions',
        'saved_suggestion' => 'Saved',
        'confirm_title' => 'Finish order',
        'finish_lede' => 'Check your details and place the order.',
        'address_name' => 'Full name',
        'company' => 'Company',
        'line1' => 'Address line 1',
        'line2' => 'Address line 2',
        'city' => 'City',
        'region' => 'State / region',
        'postal_code' => 'Postal code',
        'country' => 'Country',
        'phone' => 'Phone',
        'same_as_billing' => 'Same as billing address',
        'save_address' => 'Save this billing address to my account',
        'shipping_method' => 'Shipping method',
        'shipping_cost' => 'Shipping',
        'subtotal' => 'Subtotal',
        'no_shipping_methods' => 'No shipping methods are available for this destination.',
        'payment' => 'Payment',
        'manual_payment' => 'Pay later',
        'development_payment' => 'Pay now',
        'payment_manual' => 'Pay later',
        'payment_development' => 'Pay now',
        'manual_pending' => 'Awaiting offline payment confirmation.',
        'development_disabled' => 'Development instant payment is not enabled.',
        'place_order' => 'Place order',
        'order_summary' => 'Order summary',
        'coupon_code' => 'Discount code',
        'apply_coupon' => 'Apply',
        'coupon_applied' => 'Code :code applied.',
        'discount' => 'Discount',
        'tax' => 'Tax',
        'tax_included' => 'Tax included',
        'store_credit' => 'Account balance',
        'apply_store_credit' => 'Use account balance (:amount)',
        'credit_applied' => 'Account balance',
        'amount_due' => 'Amount due',
        'total' => 'Total',
    ],

    'countries' => [],

    'confirmation' => [
        'page_title' => 'Order confirmed',
        'kicker' => 'Order complete',
        'title' => 'Order confirmed',
        'lede' => 'Order <strong>:number</strong> is confirmed.',
        'next' => 'What happens next',
        'items' => 'Items',
        'total' => 'Total: :amount',
        'note' => 'Keep your order number for reference. Signed-in customers can also find orders under My account.',
        'back' => 'Back to catalog',
        'cards' => [
            'shipping' => [
                'title' => 'Shipping',
                'text' => 'Physical items will be prepared for shipment. Tracking appears in your account when available.',
            ],
            'physical' => [
                'title' => 'Fulfillment',
                'text' => 'The merchant will fulfill the physical items in this order.',
            ],
            'digital' => [
                'title' => 'Downloads',
                'text' => 'Digital files appear in your account when they are ready. Availability depends on payment.',
            ],
            'subscription' => [
                'title' => 'Subscription',
                'text' => 'Subscription status is shown in your account after payment is confirmed.',
            ],
            'hosting' => [
                'title' => 'Service provisioning',
                'text' => 'Hosting setup runs after payment. This can take a little time and is not instant.',
            ],
            'event' => [
                'title' => 'Tickets',
                'text' => 'Tickets appear in your account when the order is paid.',
            ],
        ],
    ],

    'payment_status' => [
        'page_title' => 'Payment status',
        'waiting' => 'Waiting for confirmation from the payment provider. This page updates automatically.',
        'checkout_event' => [
            'failed' => 'Paddle reported a failed payment attempt. No payment has been confirmed. You can try again from the order.',
            'cancelled' => 'The Paddle checkout was closed. Your order has not been confirmed as paid.',
        ],
        'view_order' => 'View order',
        'retry' => 'Pay now',
        'title' => [
            'pending' => 'Payment pending',
            'paid' => 'Payment successful',
            'failed' => 'Payment failed',
            'cancelled' => 'Payment cancelled',
            'expired' => 'Payment expired',
        ],
        'lede' => [
            'pending' => 'Order :number is waiting for payment confirmation.',
            'paid' => 'Order :number has been paid.',
            'failed' => 'Payment for order :number could not be completed.',
            'cancelled' => 'Payment for order :number was cancelled.',
            'expired' => 'Payment for order :number expired. You can try again.',
        ],
    ],

    'product' => [
        'quantity' => 'Quantity',
        'decrease' => 'Decrease quantity',
        'increase' => 'Increase quantity',
        'buy_now' => 'Buy now',
        'working' => 'Working…',
        'add_to_cart' => 'Add to cart',
        'adding' => 'Adding…',
        'choose_option' => 'Choose',
        'enable_option' => 'Yes',
        'gallery_prev' => 'Scroll thumbnails left',
        'gallery_next' => 'Scroll thumbnails right',
        'show_image' => 'Show image :number',
        'rating_aria' => ':rating out of 5 from :count reviews',
        'view_reviews' => '{0} View :count reviews|{1} View :count review|[2,*] View :count reviews',
        'reviews_count' => '{0} :count reviews|{1} :count review|[2,*] :count reviews',
        'delivery_title' => 'Delivery',
        'delivery_text' => 'Shipping options are shown at checkout.',
        'returns_title' => 'Returns',
        'returns_text' => 'Return details are set by the merchant.',
        'tabs_aria' => 'Product information',
        'tab_details' => 'Details',
        'tab_reviews' => 'Reviews',
        'about' => 'About this product',
        'spec_overview' => 'Overview',
        'spec_specifications' => 'Specifications',
        'spec_category' => 'Category',
        'spec_sku' => 'SKU',
        'spec_currency' => 'Currency',
        'spec_availability' => 'Availability',
        'spec_available' => 'Available',
        'spec_unavailable' => 'Unavailable',
        'not_available_in_currency' => 'Not available in your currency',
        'back_in_stock_title' => 'Get notified when available',
        'back_in_stock_text' => 'Enter your email and we will let you know when this product is back in stock.',
        'back_in_stock_email' => 'Email address',
        'back_in_stock_action' => 'Notify me',
        'back_in_stock_subscribed' => 'You will receive an email when this product is back in stock.',
        'back_in_stock_rate_limited' => 'Please wait before requesting another stock notification.',
        'related' => 'Related products',
        'view_product' => 'View product',
        'view_named' => 'View :name',
        'reviews_sort_newest' => 'Newest',
        'reviews_empty_title' => 'No reviews yet',
        'reviews_empty_text' => 'Be the first to share how this product holds up. Review writing ships with the customer portal.',
        'rating_summary_aria' => 'Rating summary',
    ],

    'catalog' => [
        'filter_label' => 'Filter in category',
        'filter_placeholder' => 'Filter products',
        'sort' => 'Sort',
        'sort_name' => 'Name',
        'sort_price_asc' => 'Price: low to high',
        'sort_price_desc' => 'Price: high to low',
        'apply' => 'Apply',
        'empty_title' => 'No products yet',
        'empty_text' => 'Published products will appear here. Run <code>php artisan agovena:seed-demo</code> locally.',
        'section_empty_text' => 'Published products will appear in this section.',
        'empty_category_title' => 'No products in this category',
        'back_to_home' => 'Back to home',
        'pagination' => 'Product pages',
        'previous' => 'Previous',
        'next' => 'Next',
    ],

    'home' => [
        'trust_aria' => 'Why shop here',
    ],

    'categories' => [
        'title' => 'Categories',
        'lede' => 'Browse collections and jump into a product group.',
        'empty_title' => 'No categories yet',
        'empty_text' => 'Categories will appear here once they are published.',
        'items' => '{0} :count items|{1} :count item|[2,*] :count items',
    ],

    'footer' => [
        'shop' => 'Shop',
        'account' => 'Account',
        'explore' => 'Explore',
        'legal' => 'Legal',
        'rights' => 'All rights reserved.',
    ],

    'flash' => [
        'added_to_cart' => 'Added to cart.',
        'unavailable_removed' => 'Unavailable items were removed from your cart.',
    ],

    'errors' => [
        'product_unavailable' => 'This product is not available for purchase.',
        'product_currency_unavailable' => 'This product is not available in your selected currency.',
        'cart_empty' => 'Your cart is empty.',
        'development_payment_unavailable' => 'Development payment is not available.',
        'payment_method_unavailable' => 'The selected payment method is not available.',
        'payment_gateway_required' => 'A payment gateway is required for the remaining balance. Install and enable a payment extension, or cover the full amount with account balance.',
        'automatic_renewal_unavailable' => 'Automatic renewal is only available with a payment method that supports recurring charges. Choose manual renewal or select Mollie.',
        'account_balance_insufficient' => 'You do not have enough account balance to pay this order.',
        'payment_unavailable' => 'This order cannot be paid right now.',
        'already_paid' => 'This order is already paid.',
        'shipping_address_required' => 'A shipping address is required for shippable items.',
        'shipping_method_required' => 'Please choose a shipping method.',
        'shipping_method_unavailable' => 'The selected shipping method is not available.',
        'discount_invalid' => 'This discount code is invalid.',
        'discount_expired' => 'This discount code is not currently valid.',
        'discount_exhausted' => 'This discount code has reached its usage limit.',
        'discount_customer_limit' => 'You have already used this discount code the maximum number of times.',
        'discount_minimum' => 'Your subtotal does not meet this discount code’s minimum.',
        'discount_currency' => 'This discount code is not valid for your cart currency.',
        'product_option_required' => 'Please choose :option.',
        'product_option_invalid' => 'The selected :option is not valid.',
        'product_configuration_required' => 'This cart has products that still need configuration.',
    ],

    'account_coming_soon' => 'Customer accounts will be available when the customer portal ships',
];
