...
use App\Agovena\Recurring\Events\SubscriptionCancelled;
...
final class SubscriptionService implements ProcessesSubscriptionRenewals
{
        ...
        try {
            ...
            foreach ($ids as $id) {
                ...
                DB::transaction(function () use ($id, $now, &$processed): void {
                    ...
                    if ($dueAt !== null && $this->isManualRenewal($subscription)
                        && CarbonImmutable::parse($dueAt)->lessThanOrEqualTo($now->addDays($this->invoiceLeadDays()))) {
                        ...
                        if (CarbonImmutable::parse($dueAt)->lessThanOrEqualTo($now)) {
                            $this->markPastDue($subscription->fresh() ?? $subscription);
                        }
                        $processed++;
    ...
    private function processConsolidatedGroup(
        ...
        if ($group->count() < 2 || $group->first()?->renewal_mode === 'manual') {
            return [];
        }
        ...
        $result = DB::transaction(function () use ($ids, $now, $windowEnd): ?array {
            $locked = Subscription::query()
                ...
                ->lockForUpdate()
                ->get();
            $eligible = $locked->filter(function (Subscription $subscription) use ($windowEnd): bool {
    ...
    private function consolidationKey(Subscription $subscription): string
    {
        return implode('|', [
            ...
            (string) ($subscription->renewal_mode ?? 'automatic'),
    ...
    public function processRenewalCharge(Subscription $subscription, Order $order, bool $isNewOrder, ?CarbonImmutable $now = null): RecurringChargeResult
    {
        ...
        if ($this->isManualRenewal($subscription) || ! $this->autoChargeEnabled()) {
    ...
    private function isManualRenewal(Subscription $subscription): bool
    {
        return (string) ($subscription->renewal_mode ?? 'automatic') === 'manual';
    }

    private function invoiceLeadDays(): int
    {
        return max(0, min(31, (int) $this->settings->get('store', 'subscription_invoice_lead_days', 7)));
    }
