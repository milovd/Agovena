...
async function fillDetails(page) {
    await page.getByLabel('Name', { exact: true }).fill('Ada Guest');
    await page.getByLabel('Name', { exact: true }).blur();
    await page.getByLabel('Email', { exact: true }).fill(`ada-review-${Date.now()}@example.test`);
    ...
    await page.getByLabel('Address line 1').fill('Keizersgracht 1');
    await page.getByLabel('Address line 1').blur();
    await page.getByLabel('Postal code').fill('1015 CJ');
...
await physical.getByTestId('checkout-continue').click();
...
await configure.getByTestId('checkout-continue').click();
