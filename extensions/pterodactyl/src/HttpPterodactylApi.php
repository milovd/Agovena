...
final class HttpPterodactylApi implements PterodactylApi
{
    ...
    private function request(
        ...
        $timeout = max(1, (int) $this->settings->get('pterodactyl', 'timeout', 15));
        $verify = filter_var($this->settings->get('pterodactyl', 'verify_tls', true), FILTER_VALIDATE_BOOLEAN);
    ...
    {
        $key = trim((string) $this->settings->get('pterodactyl', 'application_api_key', ''));
        if ($key === '') {
    ...
    {
        $key = trim((string) $this->settings->get('pterodactyl', 'client_api_key', ''));
