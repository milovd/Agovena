...
final class PterodactylProvisioner implements ConfiguresProvisionedProducts, Provisioner, ProvisionerActions, ProvisionerLifecycle, ProvisionerPanel
{
    ...
    {
        return [
            new ExtensionSettingDefinition('location_id', 'pterodactyl::messages.product.location_id', help: 'pterodactyl::messages.product.location_id_help'),
            new ExtensionSettingDefinition('nest_id', 'pterodactyl::messages.product.nest_id', help: 'pterodactyl::messages.product.nest_id_help'),
            new ExtensionSettingDefinition('egg_id', 'pterodactyl::messages.product.egg_id', help: 'pterodactyl::messages.product.egg_id_help'),
    ...
    public function provision(ServiceInstanceInfo $instance): void
    {
        ...
        try {
            $existing = $this->api->findServerByExternalId($externalId);
            if ($existing !== null) {
            ...
            $egg = $this->api->getEgg($nestId, $eggId);
            $created = $this->api->createServer($this->createPayload($instance, $settings, $egg, $externalId, $locationId, $eggId));
    ...
    {
        $this->mutate($instance, fn (PterodactylServer $mapping) => $this->api->suspend($mapping->server_id));
    }

    public function unsuspend(ServiceInstanceInfo $instance): void
    {
        $this->mutate($instance, fn (PterodactylServer $mapping) => $this->api->unsuspend($mapping->server_id));
    }
    ...
    public function terminate(ServiceInstanceInfo $instance): void
    {
        ...
        try {
            $this->api->delete($mapping->server_id);
        } catch (PterodactylProviderException $exception) {
    ...
    public function changePlan(ServiceInstanceInfo $instance, string $plan): void
    {
        ...
        try {
            $server = $this->api->getServer($mapping->server_id);
            $this->api->updateBuild($mapping->server_id, [
    ...
    public function syncStatus(ServiceInstanceInfo $instance): ServiceInstanceInfo
    {
        try {
            $mapping = $this->mapping($instance->id);
            if ($mapping === null) {
                $existing = $this->api->findServerByExternalId($this->externalId($instance->id));
                if ($existing === null) {
                    return $instance;
                }
    ...
    {
        if ($instance->status !== 'active' || ! $this->hasClientKey()) {
            return [];
        }
    ...
    public function runAction(ServiceInstanceInfo $instance, string $actionId): void
    {
        ...
        try {
            $this->api->power($mapping->identifier, $actionId);
        } catch (PterodactylProviderException $exception) {
            throw ValidationException::withMessages([
    ...
    public function panel(ServiceInstanceInfo $instance): ?ProvisionerPanelData
    {
        ...
        try {
            ...
            if ($this->hasClientKey()) {
                try {
                    $client = $this->api->clientServer($mapping->identifier);
        ...
        try {
            $panel = PterodactylPanelUrl::normalize($this->panelUrl());
            $fields[] = [
    ...
    {
        return trim((string) $this->settings->get('pterodactyl', 'panel_url', ''));
    }

    private function hasClientKey(): bool
    {
        return trim((string) $this->settings->get('pterodactyl', 'client_api_key', '')) !== '';
    }

    private function assertConfigured(): void
    {
        if ($this->panelUrl() === '' || trim((string) $this->settings->get('pterodactyl', 'application_api_key', '')) === '') {
            throw ValidationException::withMessages([
                'instance' => __('pterodactyl::messages.errors.not_configured'),
            ]);
        }

        if ((int) $this->settings->get('pterodactyl', 'user_id', 0) < 1) {
            throw ValidationException::withMessages([
